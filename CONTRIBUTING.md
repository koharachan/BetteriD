...
and run `pnpm run build` to generate the `en.min.json` file automatically. Only
...
`pnpm run translations` can be used to pull the latest translations from Transifex.
...
[/modules/ui/panes/help.js](/modules/ui/panes/help.js) and call `pnpm run build`.
...
JavaScript and Typescript code should pass through [ESLint](http://eslint.org/) (`pnpm run lint`) with no warnings. In case you do get errors or warnings, you can run `pnpm run lint:fix` to apply some code style fixes, eslint can perform.
...
2. run `pnpm install`
3. run `pnpm test` to see whether your tests pass or fail. Note that this command will run constantly and automatically re-run tests if source or test files are modified – if you want to run tests only once, use `pnpm run test:once` instead
...
1. Try your change locally.  Run `pnpm start` and visit `http://127.0.0.1:8080` in a browser.
2. Run lint and tests with `pnpm test`.
