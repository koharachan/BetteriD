...
实测（边缘用 `--resolve` 固定、24 张深圳附近真实瓦片并发 8、边缘 203.160.55.7）：

| 场景 | 结果 |
| --- | --- |
| 大文件（iD.min.js 604KB） | 0.62s ≈ 3.8 Mbps（CN2 机打：0.02s，命中） |
| 大文件（nsi.min.json 1.5MB） | 0.47s ≈ 8.1 Mbps（CN2 机打 ≈ 9.8 Mbps，被其 10Mbps 下行封顶） |
| 瓦片 24 张并发 8 | 0.47s ≈ 50.6 张/s / 5.5 Mbps（源站打：1.9s ≈ 12.9 张/s / 1.4 Mbps） |
| 同样资源命中慢节点 186.244.250.x | 604KB 要 4.1–7.8s ≈ 0.07–0.15 Mbps |

**边缘选择比什么都重要**（同站点差 100 倍以上）：瓦片小、看 RTT（0.12s ↔ 0.58s），
大文件看带宽（好边缘 8–10 Mbps，慢节点 0.1 Mbps）。
