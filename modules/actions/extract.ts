...
const keysToCopyAndRetain: TagKey[] = ['source', 'wheelchair'];
const keysToRetain: TagKey[] = ['area'];
const buildingKeysToRetain: TagKey[] = [
    'architect', 'building', 'height', 'layer',
    'nycdoitt:bin', 'ref:GB:uprn', 'ref:linz:building_id'
];

function isAddressKey(key: TagKey): boolean {
    return /^addr:.{1,}/.test(key);
}

function isBuildingKey(key: TagKey): boolean {
...
/**
 * Determines whether extracting `entity` should move its `addr:*` tags onto the
 * newly extracted node instead of leaving them on the building as well.
 *
 * This is only true for an address-only building (or `building:part`): it must
 * carry at least one `addr:*` tag and no other "significant" tag. Buildings
 * that also carry POI tags keep the upstream copy-and-retain behaviour.
 */
export function actionExtractMovesAddressTags(entity: OsmEntity): boolean {
    const isBuilding = (entity.tags.building && entity.tags.building !== 'no') ||
        (entity.tags['building:part'] && entity.tags['building:part'] !== 'no');

    if (!isBuilding) return false;

    let hasAddressTags = false;
