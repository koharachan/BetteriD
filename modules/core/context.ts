...
/** An edit-policy violation, as reported by the fork's military-area guard. */
export type EditPolicyViolation = NonNullable<ReturnType<typeof utilMilitaryEditViolation>>;

type EventMap = {
    enter: [];
    exit: [];
    change: [];
    blockedEdit: [violation: EditPolicyViolation];
}
...
export interface coreContext extends Pick<Dispatch<object, EventMap>, 'on'> {
    ...
    /** returns a violation when the edit policy forbids changing `before` into `after` */
    editPolicy(before: coreGraph, after: coreGraph): EditPolicyViolation | null;
    blockEdit(violation: EditPolicyViolation): void;
...
export function coreContext(this: object): coreContext {
  ...
  // The fork's edit policy: block edits that touch protected military areas,
  // except while the intro/walkthrough is running.
