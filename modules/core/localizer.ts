...
export class coreLocalizer {
    ...
    // BetteriD locale overrides: strings that the fork layers on top of the
    // translations shipped with iD.  They are deep-merged into `_localeStrings`
    // once all of the locale data has been loaded.
    // (Loaded from `data/locale_overrides.min.json` and
    //  `data/betterid_locale_overrides.min.json`, see `fileFetcher`.)
    private _localeOverrides: Translations = {};
    ...
    ensureLoaded() {
        ...
        // The first four files are required: languages, locales, and the two
        // override files.  The scope indexes are supplemental and are allowed
        // to fail, so that a missing one (for example preset translations)
        ...
        return this._loadPromise = Promise.all([
                ...
                fileFetcher.get('locale_overrides'),
                fileFetcher.get('betterid_locale_overrides'),
                ...filesToFetch.map(key => fileFetcher.get(key).catch(err => {
                    console.warn(`Unable to load translation index "${key}"`, err);  // eslint-disable-line
                    return null;
                }))
            ]).then(([languages, locales, localeOverrides, betteridLocaleOverrides, ...indexes]) => {
                this._dataLanguages = languages;
                this._dataLocales = locales;
                this._localeOverrides = localeOverrides;
                this._deepMerge(this._localeOverrides, betteridLocaleOverrides);
    ...
    // Applies the BetteriD locale overrides on top of the loaded locale strings.
    private _applyLocaleOverrides() {
        for (const scopeID of Object.keys(this._localeOverrides)) {
            const locales = this._localeOverrides[scopeID];
            ...
            for (const locale of Object.keys(locales)) {
                ...
                if (!this._localeStrings[scopeID][locale]) this._localeStrings[scopeID][locale] = {};
                this._deepMerge(this._localeStrings[scopeID][locale], strings);
            }
        }
    }


    // Deep-merges `source` into `target`.  Arrays are treated as leaf values,
    // so an override replaces a translated array instead of merging into it.
    private _deepMerge(target: Record<string, unknown>, source: Record<string, unknown>) {
